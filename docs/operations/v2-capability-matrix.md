...
| Assets, Accounts, and category versions | implemented | V2 ledger | [catalog command integration](../../backend/tests/v2/postgres/test_catalog_commands.py) | [V2 catalog models](../../backend/app/track_anywhere/infrastructure/db/models/catalog.py) | Account lifecycle includes explicit close/reopen; a credit-card liability can close only at an independently verified zero balance. |
...
| Journal | implemented | V2 ledger | [journal API contract](../../backend/tests/v2/contract/test_v2_journal_api.py) and [typed card integration](../../backend/tests/v2/postgres/test_credit_card_transactions.py) | [post transaction command](../../backend/app/track_anywhere/application/journal/post_transaction.py) and [semantic card commands](../../backend/app/track_anywhere/application/credit_cards/record.py) | Includes strict credit-card liability/subtype, natural balance, charge/payment/refund/fee, refund relation, reversal and deterministic cutover. Card profile, statement, due-date and minimum-payment products remain separate. |
...
| Payment instruments and tools | implemented | V2 ledger | [generic payment-instrument orchestration](../../backend/tests/v2/postgres/test_everyday_entry_orchestration.py) | [payment-instrument binding service](../../backend/app/track_anywhere/application/payment_instruments/service.py) | Physical/virtual form factor and network are descriptive. Immediate/prepaid instruments bind asset accounts; statement instruments bind credit-card liabilities. Full PAN, CVV, PIN, and credentials are outside the contract. |
