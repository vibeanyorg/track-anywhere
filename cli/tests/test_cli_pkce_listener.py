...
def test_auth_login_auto_listens_for_pkce_callback(monkeypatch, capsys):
    ...
    generated = iter([state, "verifier_value_12345678901234567890123456789012345678901234567890"])
    ...
    assert main(["--base-url", "http://api.test", "auth", "login", "--no-browser", "--json"]) == 0

    assert saved["token"] == "ta_cli_access"
    output = capsys.readouterr()
    assert "Waiting for browser callback on http://127.0.0.1:65123/callback" in output.err
