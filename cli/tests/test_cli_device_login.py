...
def test_auth_login_device_non_interactive_returns_authorization_payload(monkeypatch, capsys):
    def fake_request(config, method, path, payload=None, key=None):
        ...
        assert payload == {"client_id": "track-anywhere-web", "scope": "account:read book:read ledger:read"}
        return 200, {
            "device_code": "device-code-1",
            "user_code": "ABCD-EFGH",
            "verification_uri": "http://api.test/api/v1/auth/device",
            "verification_uri_complete": "http://api.test/api/v1/auth/device?user_code=ABCD-EFGH",
            "expires_in": 900,
            "interval": 5,
        }
    ...
    assert main(["--base-url", "http://api.test", "auth", "login", "--device", "--agent"]) == 0
