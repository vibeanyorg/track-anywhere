...
class ClusterConfig:
    ...
    def from_env(cls, environment: Mapping[str, str] | None = None) -> ClusterConfig:
        ...
        if values.get(EXTERNAL_DATABASE_GATE_ENV) != "1":
            raise ValueError(
                f"{EXTERNAL_DATABASE_GATE_ENV}=1 is required for the PostgreSQL 17 test lane"
            )
        ...
        if len(addresses) != 1:
            raise ValueError(
                "admin, migrator, and runtime URLs must use the same loopback host and port"
            )
        if len({admin.database, migrator.database, runtime.database}) != 1:
            raise ValueError(
                "admin, migrator, and runtime URLs must use the same base database path"
            )

        identities = {admin.username, migrator.username, runtime.username}
        if len(identities) != 3:
            raise ValueError(
                "admin, migrator, and runtime URLs must use three distinct login identities"
            )
        ...
        if migrator_role != migrator.username:
            raise ValueError(
                "migrator URL login must match TRACK_ANYWHERE_MIGRATOR_ROLE"
            )
        ...
        if len({owner_role, migrator_role, runtime_role, admin.username}) != 4:
            raise ValueError(
                "owner, admin, migrator, and runtime identities must be distinct"
            )
    ...
    def database_name_from_drop_url(self, value: str) -> str:
        ...
        if _cluster_address(url) != _cluster_address(self.admin_base_url):
            raise ValueError(
                "drop URL must use the configured loopback PostgreSQL cluster"
            )
        for base_url in (self.migrator_base_url, self.runtime_base_url):
            ...
            raise ValueError(
                "drop URL must use the configured migrator or runtime identity"
            )
        if not database_name.startswith(_DATABASE_PREFIX):
            raise ValueError(
                f"drop URL must name a database in the {_DATABASE_PREFIX} test namespace"
            )
...
class PostgresDatabaseFactory:
    def __init__(
        self, config: ClusterConfig, *, worker_id: str, test_uuid: str
    ) -> None:
        self.config = config
        self.worker_id = _validate_identifier_component(
            _slug(worker_id), label="pytest worker id"
        )
        self.test_uuid = _validate_identifier_component(
            _slug(test_uuid), label="test UUID"
        )[:16]
    ...
    def create(self, *, purpose: str, schema: str = "empty") -> ProvisionedDatabase:
        ...
        handle = ProvisionedDatabase(
            ...
            migrator_url=self.config.child_url(
                self.config.migrator_base_url, database_name
            ),
            runtime_url=self.config.child_url(
                self.config.runtime_base_url, database_name
            ),
        )
    ...
    def drop(self, database: ProvisionedDatabase | str) -> None:
        database_name = (
            ...
            if isinstance(database, ProvisionedDatabase)
            else database
        )
        if not database_name.startswith(_DATABASE_PREFIX):
            raise ValueError(
                f"refusing to drop a database outside the {_DATABASE_PREFIX} test namespace"
            )
        self._drop_database(database_name)
    ...
    def _verify_cluster_roles(self) -> None:
        ...
        try:
            with engine.connect() as connection:
                ...
                if missing:
                    raise RuntimeError(
                        f"required V2 PostgreSQL role(s) missing: {', '.join(sorted(missing))}"
                    )
                ...
                if owner.rolcanlogin or any(
                    getattr(owner, attribute) for attribute in forbidden_attributes
                ):
                    raise RuntimeError(
                ...
                owner_memberships = [
                    ...
                    if membership["member_role"] == self.config.owner_role
                ]
                if owner_memberships:
                    raise RuntimeError(
                        "V2 owner role must not have any direct role membership"
                    )
                ...
                if runtime_memberships:
                    raise RuntimeError(
                        "V2 runtime role must not have any direct role membership"
                    )
                ...
                if (
                    len(migrator_memberships) != 1
                    or dict(migrator_memberships[0]) != expected_membership
                ):
                    raise RuntimeError(
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Provision disposable Track Anywhere V2 test databases"
    )
    commands = parser.add_subparsers(dest="command", required=True)

    create = commands.add_parser("create")
    create.add_argument("--purpose", required=True)
    create.add_argument("--schema", required=True, choices=("empty", "v2"))
    create.add_argument("--emit-role", required=True, choices=("migrator", "runtime"))

    role_name = commands.add_parser("role-name")
    role_name.add_argument(
        "--kind", required=True, choices=("owner", "migrator", "runtime")
    )
...
def _emit_stdout(value: str) -> bool:
    try:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if arguments.command == "role-name":
        return 0 if _emit_stdout(factory.role_name(arguments.kind)) else 1
    if arguments.command == "create":
        database = factory.create(purpose=arguments.purpose, schema=arguments.schema)
        delivered = _emit_stdout(
            database.migrator_url
            if arguments.emit_role == "migrator"
            else database.runtime_url
        )
        if not delivered:
