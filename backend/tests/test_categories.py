...
def test_categories_start_empty_and_support_two_levels(tmp_path):
    ...
    child, _ = service.create_category(
        service.owner_token,
        {"kind": "expense", "name": "外卖", "parent_id": category.category_id},
    ...
    category_tree = service.list_categories(service.owner_token, kind="expense")

    assert category_tree == [category, child]
    assert category.level == 1
    assert child.level == 2
    assert child.parent_id == category.category_id
    assert child.path_cache == "餐饮 / 外卖"


def test_duplicate_categories_are_rejected(tmp_path):
    service = FinanceService(DeploymentSecurityConfig(), database_url=f"sqlite:///{tmp_path / 'track-anywhere.sqlite3'}")
    payload = {"kind": "income", "name": "工资"}
...
def test_record_expense_and_income_with_categories_and_summary(tmp_path):
    ...
    lunch_category, _ = service.create_category(
        service.owner_token,
        {"kind": "expense", "name": "午餐", "parent_id": expense_category.category_id},
        idempotency_key="cat-lunch",
    )
    income_category, _ = service.create_category(
        service.owner_token,
        {"kind": "income", "name": "工资"},
        idempotency_key="cat-salary",
    )
    job_category, _ = service.create_category(
        service.owner_token,
        {"kind": "income", "name": "主业", "parent_id": income_category.category_id},
    ...
    expense_tx, _ = service.record_expense(
        service.owner_token,
        {
            ...
            "category_id": lunch_category.category_id,
            "purpose": "lunch",
    ...
    assert expense_tx.lines[0].category_id == lunch_category.category_id
    assert income_tx.lines[0].category_id == job_category.category_id
    ...
    assert service.list_transactions(service.owner_token, category_id=lunch_category.category_id) == [expense_tx]
...
def test_categories_and_transaction_links_persist(tmp_path):
    ...
    category, _ = first.create_category(
        ...
        idempotency_key="persist-category-parent",
    )
    child, _ = first.create_category(
        token,
        {"kind": "expense", "name": "打车", "parent_id": category.category_id},
