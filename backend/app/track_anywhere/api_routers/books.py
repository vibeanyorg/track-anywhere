...
def list_books(token: AuthToken, service: BookService):
    try:
        return {"books": serialize(service.list_books(token))}
    except COMMAND_ERRORS as exc:
        raise_command_error(exc, "book.list", recorder=service)


@router.post("", dependencies=protected)
def create_book(payload: CreateBookCommand, token: AuthToken, key: IdempotencyKey, service: BookService):
    try:
        ...
        raise_command_error(exc, "book.create", recorder=service)


@router.get("/{book_id}", dependencies=protected)
def get_book(book_id: str, token: AuthToken, service: BookService):
    try:
        return {"book": serialize(service.get_book(token, book_id))}
    except COMMAND_ERRORS as exc:
        raise_command_error(exc, "book.get", recorder=service)
...
def list_book_transactions(book_id: str, token: AuthToken, service: BookService, limit: int = 20):
    try:
        return {"transactions": serialize(service.list_book_transactions(token, book_id, limit=limit))}
    except COMMAND_ERRORS as exc:
        raise_command_error(exc, "book.transaction.list", recorder=service)


@router.post("/{book_id}/transactions", dependencies=protected)
def record_book_transaction(
    book_id: str,
    ...
    try:
        ...
        raise_command_error(exc, "book.transaction.record", recorder=service)


@router.post("/{book_id}/transactions/{transaction_id}/reverse", dependencies=protected)
def reverse_book_transaction(
    book_id: str,
    transaction_id: str,
    payload: ReverseBookTransactionCommand,
    token: AuthToken,
    key: IdempotencyKey,
    service: BookService,
):
...
def list_book_categories(
    ...
    try:
        categories = service.list_book_categories(token, book_id, kind=kind, name=name, parent_id=parent_id)
        return {"categories": serialize(categories)}
    except COMMAND_ERRORS as exc:
        raise_command_error(exc, "book.category.list", recorder=service)
...
def create_book_category(
    ...
    try:
        ...
        raise_command_error(exc, "book.category.create", recorder=service)
...
def update_book_category(
    ...
    try:
        category, replay = service.update_book_category(
            token,
            book_id,
            category_id,
            command_payload(payload),
        ...
        raise_command_error(exc, "book.category.update", recorder=service)
...
def add_book_category_alias(
    book_id: str,
    category_id: str,
    payload: AddCategoryAliasCommand,
    token: AuthToken,
    key: IdempotencyKey,
    service: BookService,
):
    try:
        alias, replay = service.add_book_category_alias(
            token,
            book_id,
            category_id,
            command_payload(payload),
        ...
        raise_command_error(exc, "book.category.alias.add", recorder=service)
...
def merge_book_category(
    book_id: str,
    category_id: str,
    payload: MergeCategoryCommand,
    token: AuthToken,
    key: IdempotencyKey,
    service: BookService,
):
    try:
        category, replay = service.merge_book_category(
            token,
            book_id,
            category_id,
            command_payload(payload),
        ...
        raise_command_error(exc, "book.category.merge", recorder=service)


@router.get("/{book_id}/classification-events", dependencies=protected)
def list_book_classification_events(book_id: str, token: AuthToken, service: BookService):
    try:
        return {"events": serialize(service.list_book_classification_events(token, book_id))}
    except COMMAND_ERRORS as exc:
        raise_command_error(exc, "book.classification_event.list", recorder=service)


@router.get("/{book_id}/budgets", dependencies=protected)
def list_book_budgets(book_id: str, token: AuthToken, service: BookService):
    try:
        return {"budgets": serialize(service.list_budgets(token, book_id))}
    except COMMAND_ERRORS as exc:
        raise_command_error(exc, "book.budget.list", recorder=service)


@router.post("/{book_id}/budgets", dependencies=protected)
def create_book_budget(
    book_id: str,
    ...
    try:
        ...
        raise_command_error(exc, "book.budget.create", recorder=service)


@router.post("/{book_id}/budgets/{budget_id}/targets", dependencies=protected)
def add_book_budget_target(
    book_id: str,
    budget_id: str,
    payload: CreateBudgetTargetCommand,
    token: AuthToken,
    key: IdempotencyKey,
    service: BookService,
):
    try:
        target, replay = service.add_budget_target(
            token,
            book_id,
            budget_id,
            command_payload(payload),
        ...
        raise_command_error(exc, "book.budget.target.add", recorder=service)


@router.get("/{book_id}/budgets/{budget_id}/targets", dependencies=protected)
def list_book_budget_targets(book_id: str, budget_id: str, token: AuthToken, service: BookService):
    try:
        return {"budget_targets": serialize(service.list_budget_targets(token, book_id, budget_id))}
    except COMMAND_ERRORS as exc:
        raise_command_error(exc, "book.budget.target.list", recorder=service)


@router.get("/{book_id}/budgets/{budget_id}/execution", dependencies=protected)
def get_book_budget_execution(book_id: str, budget_id: str, token: AuthToken, service: BookService):
    try:
        return service.budget_execution_report(token, book_id, budget_id)
    except COMMAND_ERRORS as exc:
        raise_command_error(exc, "book.budget.execution", recorder=service)


@router.get("/{book_id}/reports/spending", dependencies=protected)
def book_spending_report(
    book_id: str,
    token: AuthToken,
    service: BookService,
    ...
    try:
        ...
        raise_command_error(exc, "book.report.spending", recorder=service)
