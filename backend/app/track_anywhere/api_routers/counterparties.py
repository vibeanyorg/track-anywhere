...
def ensure_counterparty(
    payload: CreateCounterpartyCommand,
    token: AuthToken,
    key: IdempotencyKey,
    ...
    try:
        ...
        raise_command_error(exc, "counterparty.ensure", recorder=service)
...
def create_counterparty(
    ...
    try:
        ...
        raise_command_error(exc, "counterparty.create", recorder=service)
...
def list_counterparties(
    ...
    try:
        ...
        raise_command_error(exc, "counterparty.list", recorder=service)


@router.get("/counterparties/{counterparty_ref}", dependencies=protected)
def get_counterparty(counterparty_ref: str, token: AuthToken, service: CounterpartyService):
    try:
        return {"counterparty": serialize(service.get_counterparty(token, counterparty_ref))}
    except COMMAND_ERRORS as exc:
        raise_command_error(exc, "counterparty.get", recorder=service)
