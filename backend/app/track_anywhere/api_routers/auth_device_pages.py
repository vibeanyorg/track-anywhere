...
from ..api_ports.auth_device import AuthDeviceService
...
@router.get("/device")
def device_page(request: Request, auth_service: AuthDeviceService, user_code: str | None = None) -> HTMLResponse:
...
def device_approve(
    request: Request,
    auth_service: AuthDeviceService,
    ...
    try:
        ...
        grant = auth_service.approve_platform_device_user_code_for_actor(
            user_code,
            actor,
            action,
        ...
        return _device_form(
            ...
            error=str(exc),
            status_code=400,
            available_scope_text=available_scope_text,
        )
    ...
    """
    return render_auth_page(f"Device {verb}", body)
...
def _device_form(
    *,
    ...
    scope_options = _device_scope_options(service, user_code, available_scope_text)
    body = f"""
      <section class="panel">
        <p class="eyebrow">Track Anywhere Device Login</p>
    ...
    """
    return render_auth_page("Authorize device", body, status_code)


def _device_scope_options(service: AuthDeviceService, user_code: str, available_scope_text: str | None) -> str:
    if not user_code.strip():
        return ""
    grant = service.pending_device_grant_for_user_code(user_code)
    if grant is None or grant.status != "pending":
        return ""
    return hidden_input("scope_selection_present", "1") + scope_controls(" ".join(grant.scopes), available_scope_text=available_scope_text)
