...
def create_payment_instrument(payload: CreatePaymentInstrumentCommand, token: AuthToken, key: IdempotencyKey):
    try:
        ...
        raise_command_error(exc, "payment_instrument.create")


@router.get("/payment-instruments", dependencies=protected)
def list_payment_instruments(token: AuthToken, account_id: str | None = None, status: str | None = "active"):
    try:
